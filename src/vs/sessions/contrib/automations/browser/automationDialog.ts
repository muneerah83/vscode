...
class AutomationIsolationGroupActionViewItem extends BaseActionViewItem {
	...
	private readonly _selectedBranch = observableValue<string | undefined>('branch', undefined);
	private readonly _branchLoading = observableValue<boolean>('branchLoading', false);
	...
	private observeRepository(folder: URI | undefined): void {
		...
		this._branchLoading.set(false, undefined);

		if (!folder) {
			return;
		}

		this._branchLoading.set(true, undefined);
